...
<div style="margin:12px 20px;">
   <form method="post" action="/index.php/Admin/Categery/upd">
	<input type="hidden" name="cat_id" value="<?php echo ($info["cat_id"]); ?>"/>
    <table border="1" width="100%">
        <tr style="font-weight:bold;">
            <td>父级分类</td><td><select name="cat_fid">
                <option value="0">顶级分类</option>
                <?php if(is_array($infos)): foreach($infos as $key=>$v): if($info['cat_fid']==$v['cat_id']): ?><option value="<?php echo ($v["cat_id"]); ?>" selected>
						 <?php else: ?>
		  ...
		  <tr style="font-weight:bold;">
            <td>子分类名</td><td><input type="text" name="cat_name" value="<?php echo ($info["cat_name"]); ?>"/></td>
        </tr>
        <tr>
