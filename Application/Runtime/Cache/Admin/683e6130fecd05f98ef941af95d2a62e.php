...
<div style="margin:12px 20px;">
   <form method="post" action="/index.php/Admin/Auth/upd">
	<input type="hidden" name="auth_id" value="<?php echo ($v["auth_id"]); ?>"/>
    <table class="tables" border="1" width="100%">
           <tr style="font-weight: bold;">
					 <td width="10%">权限名称</td><td><input type="text" name="auth_name" value="<?php echo ($v["auth_name"]); ?>"/></td></tr>
					 <tr><td width="10%">父级权限</td>
					   <td><select name="auth_pid">
						<option value="0">--请选择--</option>
                  <?php if(is_array($info)): foreach($info as $key=>$vv): if($vv['auth_id']==$v['auth_pid']): ?><option value="<?php echo ($vv["auth_id"]); ?>" selected="selected">
						  <?php else: ?>
						  <option value="<?php echo ($vv["auth_id"]); ?>"><?php endif; ?>
						  <?php echo str_repeat('&nbsp;&nbsp;&nbsp;',$vv['auth_level']); echo ($vv["auth_name"]); ?>
						  </option><?php endforeach; endif; ?>
					   </select></td></tr>
					 <tr><td width="10%">控制器</td><td><input type="text" name="auth_c" value="<?php echo ($v["auth_c"]); ?>" /></td></tr>
					 <tr><td width="10%">操作方法</td><td><input type="text" name="auth_a" value="<?php echo ($v["auth_a"]); ?>" /></td></tr>
            </tr>
				</tr>
