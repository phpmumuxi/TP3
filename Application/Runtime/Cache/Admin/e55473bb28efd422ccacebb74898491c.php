...
<div style="margin:12px 20px;">
    <table class="tables" border="1" width="100%">
           <tr style="font-weight: bold;">
                <td width="20%">序号</td><td width="20%">分类名称</td>
					 <td width="20%">父级ID</td>
                <td align="center" colspan='2' >操作</td>
            </tr>
            <?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
                <td><?php echo ($v["cat_id"]); ?></td>
                <td><?php echo str_repeat('&nbsp;&nbsp;&nbsp;',$v['level']); echo ($v["cat_name"]); ?></td>
                <td><?php echo ($v["cat_fid"]); ?></td>
                <td><a href="<?php echo U('Admin/Categery/upd',array('cat_id'=>$v['cat_id']));?>">修改</a></td>
                <td><a href="javascript:;" onclick="confirmdel(<?php echo ($v["cat_id"]); ?>)">删除</a></td>
            </tr><?php endforeach; endif; ?>
    </table>
	 <p style="margin:10px 20px;text-align:center"><?php echo ($pagelist); ?></p>
</div>
<script>
function confirmdel(id){
  if(window.confirm('你确定要删除吗？')){
    location.href="<?php echo U('Admin/Categery/del/id/"+id+"');?>";
  }
}
</script>
