...
<div style="margin:12px 20px;">
    <table class="tables" border="1" width="100%">
           <tr style="font-weight: bold;">
                <td width="10%">序号</td><td width="10%">名称</td>
					 <td width="20%">权限ID</td>
					 <td width="30%">控制器方法</td>
                <td align="center" colspan='2' >操作</td>
            </tr>
            <?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
                <td><?php echo ($v["role_id"]); ?></td>
                <td><?php echo ($v["role_name"]); ?></td>
                <td><?php echo ($v["role_all"]); ?></td>
                <td><?php echo ($v["role_ac"]); ?></td>
                <td><a href="<?php echo U('Admin/Role/upd',array('role_id'=>$v['role_id']));?>">修改</a></td>
                <td><a href="javascript:;" onclick="confirmdel(<?php echo ($v["role_id"]); ?>)">删除</a></td>
...
function confirmdel(id){
  if(window.confirm('你确定要删除吗？')){
    location.href="<?php echo U('Admin/Role/del/id/"+id+"');?>";
  }
}
</script>
