...
<div style="margin:12px 20px;">
   <form method="post" action="/index.php/Admin/Attribute/upd">
	<input type="hidden" name="attr_id" value="<?php echo ($info["attr_id"]); ?>"/>
    <table border="1" width="100%">
        <tr style="font-weight:bold;">
            <td>属性名称</td><td><input type="text" name="attr_name" value="<?php echo ($info["attr_name"]); ?>"/></td>
        </tr>
		  <tr style="font-weight:bold;">
            <td>属性值</td><td><input type="text" name="attr_value" value="<?php echo ($info["attr_value"]); ?>"/></td>
        </tr>
		  <tr style="font-weight:bold;">
            <td>所属类型</td><td><select name="type_id">
					<option value="0">-请选择-</option>
					<?php if(is_array($infos)): foreach($infos as $key=>$vo): if($info['type_id']==$vo['type_id']): ?><option value="<?php echo ($vo["type_id"]); ?>" selected><?php echo ($vo["type_name"]); ?></option>
					<?php else: ?>
					<option value="<?php echo ($vo["type_id"]); ?>"><?php echo ($vo["type_name"]); ?></option><?php endif; endforeach; endif; ?>
            </select></td>
        </tr>
        <tr>
