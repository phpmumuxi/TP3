...
<div style="margin:12px 20px;">
   <form method="post" action="/index.php/Admin/Categery/add">
    <table border="1" width="100%">
        <tr style="font-weight:bold;">
            <td>父级分类</td><td><select name="cat_fid">
                <option value="0">顶级分类</option>
                <?php if(is_array($info)): foreach($info as $key=>$v): ?><option value="<?php echo ($v["cat_id"]); ?>">
						<?php echo str_repeat('&nbsp;&nbsp;&nbsp;',$v['level']); echo ($v["cat_name"]); ?>
						  </option><?php endforeach; endif; ?>
            </select></td>
        </tr>
		  <tr style="font-weight:bold;">
            <td>子分类名</td><td><input type="text" name="cat_name" placeholder="分类别名不能重复"/></td>
        </tr>
        <tr>
